import { HighlightModel } from '../model/Highlight';
import { v4 as uuidv4 } from 'uuid';

export class HighlightSeeder {
  async run(){
    const highlights = [
      { 
        highlightId: uuidv4(),
        talentId: "dc8a0ff0-1d0e-414a-9f37-ebe70fae5fc4",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "dc8a0ff0-1d0e-414a-9f37-ebe70fae5fc4",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },      
      { 
        highlightId: uuidv4(),
        talentId: "dc8a0ff0-1d0e-414a-9f37-ebe70fae5fc4",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },

      { 
        highlightId: uuidv4(),
        talentId: "6b908512-a145-4e3b-b072-96af84b00d87",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "6b908512-a145-4e3b-b072-96af84b00d87",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },      
      { 
        highlightId: uuidv4(),
        talentId: "6b908512-a145-4e3b-b072-96af84b00d87",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },

      { 
        highlightId: uuidv4(),
        talentId: "c1124cfc-3f62-4f9a-8182-2819d0624da5",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "c1124cfc-3f62-4f9a-8182-2819d0624da5",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "c1124cfc-3f62-4f9a-8182-2819d0624da5",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },


      { 
        highlightId: uuidv4(),
        talentId: "4b00656e-374f-4b99-ae45-e079bac5f3ee",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "4b00656e-374f-4b99-ae45-e079bac5f3ee",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "4b00656e-374f-4b99-ae45-e079bac5f3ee",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },

      { 
        highlightId: uuidv4(),
        talentId: "ac56a04d-aa98-4591-8e8c-2b188040c114",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "ac56a04d-aa98-4591-8e8c-2b188040c114",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "ac56a04d-aa98-4591-8e8c-2b188040c114",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },

      { 
        highlightId: uuidv4(),
        talentId: "89341cbe-65d3-46bd-b905-32becfbbf0a4",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "89341cbe-65d3-46bd-b905-32becfbbf0a4",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "89341cbe-65d3-46bd-b905-32becfbbf0a4",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },

      { 
        highlightId: uuidv4(),
        talentId: "7f2f85fb-b6ed-4707-8763-2cc67c398b30",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "7f2f85fb-b6ed-4707-8763-2cc67c398b30",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "7f2f85fb-b6ed-4707-8763-2cc67c398b30",
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },

      { 
        highlightId: uuidv4(),
        talentId: "f0b8710a-7edd-4e67-a0fa-b3754bfd8576", 
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "f0b8710a-7edd-4e67-a0fa-b3754bfd8576", 
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
      { 
        highlightId: uuidv4(),
        talentId: "f0b8710a-7edd-4e67-a0fa-b3754bfd8576", 
        highlightURL: "https://i.pinimg.com/564x/e8/0b/f4/e80bf43ad9490efa8afdfc6b952aa7d5.jpg",
        highlightNameFile: "https://storage.googleapis.com/payroll_anggi/ABSEN-64c89bf5395a8d082f894723-f1cba4cbe148f090bb941690872092070.png"
      },
  ];

  await HighlightModel.destroy({ where: {} })
  await HighlightModel.bulkCreate(highlights.map(highlight => ({...highlight })));



  }
} 